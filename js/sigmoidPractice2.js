...
const sigmoidA = () => {
    const arr = [];
    for (let i = 0; i < 4.5; i += 0.5) arr.push(2 * (sigmoid(i + 0.5) - sigmoid(i)))
    return arr
}
...
const sigmoidB = () => {
    const arr = [];
    for (let i = 0; i < 4.5; i += 0.5) arr.push(2 * (i + 0.5) * sigmoid(i) - 2 * i * sigmoid(i + 0.5))
    return arr
}
...
const outputFilePath = '../jsResults/OutPractice2.dat';
var mse = 0;
